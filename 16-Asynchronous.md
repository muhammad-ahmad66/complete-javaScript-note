...
11. [BUILDING_A_SIMPLE_PROMISE](#building_a_simple_promise)
12. [PROMISIFYING_THE_GEOLOCATION_API](#promisifying_the_geolocation_api)
13. [Consuming_Promises_with_Async_Await](#consuming_promises_with_async_await)
14. [Error_Handling_With_Try-Catch](#error_handling_with_try-catch)
15. [Returning_Values_from_Async_Functions](#returning_values_from_async_functions)
16. [Running_promises_In_Parallel](#running_promises_in_parallel)
...
If there is only one thead of execution in the engine then how can asynchronous code be executed in a non blocking way? [FOR_BETTER_VISUALIZATION_SEE_PDF_FILE]  
Fist all asynchronous codes will be added in **WEB APIs Section**

**JS has three main parts**.

1. call stack
2. web APIs
3. Callback & microTask queue.

As any asynchronous code completes its execution it will add into **callback queue**.
...
Microtask queue has priority over callback queue. So, at the end of an event loop tick(taking callback form callback queue), the event loop check if there are any callbacks in the microtask queue. If there are, run all of them, before any more callbacks from the regular callback queue.  
...
Remember the resolve and reject both are also gonna a function.
...
const lotteryPromise = new Promise(function (resolve, reject) {
  if (Math.random() >= 0.5) {
    resolve('You WIN 💰');
    // Calling a resolve() functions like this, we mark this promise as a fulfilled(resolved).
    // And what ever we pass in resolve function is gonna be the result of the promise that will be available in the then handler.
  } else {
    reject('You Lost your money 💩');
    // Into the reject we will pass an error message, that we later want to be able to catch in catch method.
  }
});
```
...
So for that we wil simulate these codes
...
## Consuming_Promises_with_Async_Await

Better way to consuming promises in ES17, which is called **Async Await**.  
We start with creating a special kind of function, which is an async function. we do that with writing **async keyword** before any function.  
...
**NOTE:** Async await is only about consuming promises. We can't build any promise using async await.
...
// res.json(); // Remember this also return a promise. so now we use await again instead of then. and store the result into a variable.
...
**One Simple Example of Try Catch** (in regular code)
...
## Returning_Values_from_Async_Functions

Code Implementation is in above example, Just Modified.⤴
...
const get3Countries = async function (c1, c2, c3) {
  ...
  try {
    ...
    // Here All three await statements are executing one another, not in parallel.
    // Which will take so much time. oT load if there is ton of calls.
    // So we will try to execute all of these in parallel.

    // To do that we use promise.all combinator function.
    // It call a combinator function because it allow to combine all promises.
    // This promise.all function will take an array of promises and it will return a new promise
    // And it will run all the promises in the array at the same time.
